//******************************************************************************
//
// File:    FloatMatrixReductionBuf.java
// Package: benchmarks.determinism.pj.edu.ritmp.buf
// Unit:    Class benchmarks.determinism.pj.edu.ritmp.buf.FloatMatrixReductionBuf
//
...
/**
 * Class FloatMatrixReductionBuf provides a reduction buffer for class
 * {@linkplain FloatMatrixBuf}.
 *
 * @author  Alan Kaminsky
 * @version 25-Oct-2007
 */
class FloatMatrixReductionBuf
...
// Exported constructors.
	...
	/**
	 * Construct a new float matrix reduction buffer. It is assumed that the
	 ...
	 */
	public FloatMatrixReductionBuf
		(float[][] theMatrix,
		 Range theRowRange,
		 Range theColRange,
		 FloatOp op)
		...
		if (op == null)
			...
			throw new NullPointerException
				("FloatMatrixReductionBuf(): op is null");
			}
		myOp = op;
		}
	...
	protected int receiveItems
		...
		while (r < myRowCount && ncols > 0)
			...
			while (c < ncols)
				{
				myMatrix_row[col] =
					myOp.op (myMatrix_row[col], floatbuffer.get());
				++ c;
				col += myColStride;
				}
			n += ncols;
