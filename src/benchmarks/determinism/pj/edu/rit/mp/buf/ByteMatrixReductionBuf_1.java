//******************************************************************************
//
// File:    ByteMatrixReductionBuf_1.java
// Package: benchmarks.determinism.pj.edu.ritmp.buf
// Unit:    Class benchmarks.determinism.pj.edu.ritmp.buf.ByteMatrixReductionBuf_1
//
...
/**
 * Class ByteMatrixReductionBuf_1 provides a reduction buffer for class
 * {@linkplain ByteMatrixBuf_1}.
 *
 * @author  Alan Kaminsky
 * @version 25-Oct-2007
 */
class ByteMatrixReductionBuf_1
	extends ByteMatrixBuf_1
	{
...
// Exported constructors.
	...
	/**
	 * Construct a new byte matrix reduction buffer. It is assumed that the rows
	 * and columns of <TT>theMatrix</TT> are allocated and that each row of
	 ...
	 */
	public ByteMatrixReductionBuf_1
		(byte[][] theMatrix,
		 Range theRowRange,
		 Range theColRange,
		 ByteOp op)
		...
		if (op == null)
			...
			throw new NullPointerException
				("ByteMatrixReductionBuf_1(): op is null");
			}
		myOp = op;
		}
	...
	protected int receiveItems
		...
		while (r < myRowCount && ncols > 0)
			...
			while (c < ncols)
				{
				myMatrix_row[col] =
					myOp.op (myMatrix_row[col], buffer.get());
				++ c;
				++ col;
				}
			n += ncols;
