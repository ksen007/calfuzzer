//******************************************************************************
//
// File:    BooleanItemReductionBuf.java
// Package: benchmarks.determinism.pj.edu.ritmp.buf
// Unit:    Class benchmarks.determinism.pj.edu.ritmp.buf.BooleanItemReductionBuf
//
 ...
 * {@linkplain BooleanItemBuf}.
 *
 * @author  Alan Kaminsky
 * @version 25-Oct-2007
 */
class BooleanItemReductionBuf
	extends BooleanBuf
	{

// Hidden data members.

	BooleanItemBuf myBuf;
	BooleanOp myOp;

// Exported constructors.

	/**
	 * Construct a new Boolean item reduction buffer.
	 *
	 ...
	 */
	public BooleanItemReductionBuf
		(BooleanItemBuf buf,
		 BooleanOp op)
		{
		super (1);
		if (op == null)
			...
			throw new NullPointerException
				("BooleanItemReductionBuf(): op is null");
			}
		myBuf = buf;
		myOp = op;
		}
...
// Hidden operations.
	...
	protected int sendItems
		...
		if (buffer.remaining() >= 1)
			{
			buffer.put (myBuf.item ? (byte) 1 : (byte) 0);
			return 1;
			}
		else
			{
	...
	protected int receiveItems
		...
		if (num >= 1 && buffer.remaining() >= 1)
			{
			myBuf.item = myOp.op (myBuf.item, buffer.get() != 0);
			return 1;
			}
		else
			{
