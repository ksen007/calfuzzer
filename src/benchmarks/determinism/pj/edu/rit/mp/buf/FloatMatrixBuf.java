//******************************************************************************
//
// File:    FloatMatrixBuf.java
// Package: benchmarks.determinism.pj.edu.ritmp.buf
// Unit:    Class benchmarks.determinism.pj.edu.ritmp.buf.FloatMatrixBuf
//
...
/**
 * Class FloatMatrixBuf provides a buffer for a matrix of float items
 ...
 * FloatMatrixBuf may be constructed directly, normally you will use a
 * factory method in class {@linkplain benchmarks.determinism.pj.edu.ritmp.FloatBuf FloatBuf}.
...
// Exported operations.
	...
	public Buf getReductionBuf
		(Op op)
		{
		return new FloatMatrixReductionBuf
...
// Hidden operations.
	...
	protected int sendItems
		...
		while (r < myRowCount && ncols > 0)
			...
			while (c < ncols)
				{
				floatbuffer.put (myMatrix_row[col]);
				++ c;
				col += myColStride;
				}
			n += ncols;
	...
	protected int receiveItems
		...
		while (r < myRowCount && ncols > 0)
			...
			while (c < ncols)
				{
				myMatrix_row[col] = floatbuffer.get();
				++ c;
				col += myColStride;
				}
			n += ncols;
