...
public class ToResult implements java.io.Serializable {
   ...
   *
   * However, this is ok because result headers are unique.  That is,
   * there will only be one ToResult with each header per program, so
   * this equals() is safe for comparing two ToResult's from the same
   * or form two different programs.
   */
  public boolean equals(Object o) {
      ...
      if ((o == null) || (getClass() != o.getClass()))
          return false;
      ToResult tr = (ToResult)o;

      if (!header.equals(tr.header)) return false;
      if (!((Math.abs(expectedReturnRate - tr.expectedReturnRate) < 1e-10) &&
            (Math.abs(volatility - tr.volatility) < 1e-10) &&
            (Math.abs(volatility2 - tr.volatility2) < 1e-10) &&
            (Math.abs(finalStockPrice - tr.finalStockPrice) < 1e-10) &&
            (pathValue.length == tr.pathValue.length)))
          return false;
      for (int i = 0; i < pathValue.length; i++) {
          if (Math.abs(pathValue[i] - tr.pathValue[i]) >= 1e-10)
              return false;
      }
      return true;
  }
