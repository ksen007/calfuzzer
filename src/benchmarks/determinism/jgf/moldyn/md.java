...
package benchmarks.determinism.jgf.moldyn;
...
import edu.berkeley.cs.detcheck.Predicate;
...
/* spawn threads */
    ...
    requireDeterministic(Arrays.asList(new Integer[] {mm, PARTSIZE}));
    ...
    // Need to get approximate equality predicate here.
    assertDeterministic(Arrays.asList(new Double[] {epot[0], vir[0], ek[0]}),
...
/* Barrier */
   br.DoBarrier(id);
    ...
    if(id == 0) {
      ...
      assertDeterministic(sh_force, new ApproxEquals(1e-10));
    }
...
/* sum to get full potential energy and virial */
   ...
   br.DoBarrier(id);
   if (id == 0) {
       assertDeterministic(md.ek, new ApproxEquals(1e-10));
       assertDeterministic(md.epot, new ApproxEquals(1e-10));
       assertDeterministic(md.vir, new ApproxEquals(1e-10));
   }
   br.DoBarrier(id);

   /*
    * Should really assert here that every iteration has been
    * deterministic:
    *
    * if (id == 0) {
    *   assertDeterministic(md.epot);
    *   assertDeterministic(md.vir);
    *   assertDeterministic(md.interactions);
    *   assertDeterministic(md.sh_force);
    *   assertDeterministic(ekin);
    *   assertDeterministic(vel);
    *   ...
    * }
    * br.DoBarrier(id);
    *
    * And could even move assertions up to right after computation happens.
    */
   }
...
class ParticleApproxEquals implements Predicate {
    public ParticleApproxEquals(double tol) {
        this.tol = tol;
    }

    public boolean apply(Object a, Object b) {
        particle[] ap = (particle[])a;
        particle[] bp = (particle[])b;
        if (ap.length != bp.length)
            return false;

        for (int i = 0; i < ap.length; i++) {
            if (Math.abs(ap[i].xcoord - bp[i].xcoord) >= tol) return false;
            if (Math.abs(ap[i].ycoord - bp[i].ycoord) >= tol) return false;
            if (Math.abs(ap[i].zcoord - bp[i].zcoord) >= tol) return false;
            if (Math.abs(ap[i].xvelocity - bp[i].xvelocity) >= tol) return false;
            if (Math.abs(ap[i].yvelocity - bp[i].yvelocity) >= tol) return false;
            if (Math.abs(ap[i].zvelocity - bp[i].zvelocity) >= tol) return false;
        }
...
class particle implements Serializable {
  ...
  int part_id;
  int id;
  transient double [][] sh_force;
  transient double [][][] sh_force2;
  transient mdRunner runner;
